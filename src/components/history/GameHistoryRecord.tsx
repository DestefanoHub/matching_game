...
export default function GameHistoryRecord({ game, onClick }: Props) {
    ...
    return <div className={`${styles.record} ${game.hasWon ? styles.win : styles.loss}`} onClick={handleClick}>
        <h1>Game Details</h1>
        <p><span className='playerName'>{game.player.username}</span> {game.hasWon ? 'won :)' : 'lost :('} a {getDisplayDifficulty(game.difficulty as Difficulty)} game on {gameDateObj.toLocaleString('en-US')}</p>
    </div>;
}